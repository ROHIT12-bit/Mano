...
async def broadcast_cmd(client: Client, message: Message):
    ...
    if not message.reply_to_message and len(message.command) < 2:
        await message.reply_text(quote_text("Reply to a message or provide text to broadcast."), parse_mode=pyrogram.enums.ParseMode.HTML)
        return

    ms = await message.reply_text(quote_text("Broadcasting..."), parse_mode=pyrogram.enums.ParseMode.HTML)
    ...
    async for user in all_users:
        try:
            if message.reply_to_message:
                ...
                await client.send_message(user['id'], quote_text(broadcast_text), parse_mode=pyrogram.enums.ParseMode.HTML)
            success += 1
        except:
            failed += 1
...
async def ban_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await message.reply_text(quote_text("Usage: /ban [user_id]"), parse_mode=pyrogram.enums.ParseMode.HTML)
        return
    try:
        ...
        await message.reply_text(quote_text("Invalid User ID."), parse_mode=pyrogram.enums.ParseMode.HTML)
...
async def unban_cmd(client: Client, message: Message):
    ...
    if message.from_user.id not in Botskingdoms:
        ...
        return
    if len(message.command) < 2:
        await message.reply_text(quote_text("Usage: /unban [user_id]"), parse_mode=pyrogram.enums.ParseMode.HTML)
        return
    try:
        user_id = int(message.command[1])
...
async def add_admin_cmd(client: Client, message: Message):
    ...
    if message.from_user.id not in Config.Botskingdoms:
        await message.reply_text(quote_text("Only the owner can add admins."), parse_mode=pyrogram.enums.ParseMode.HTML)
        return
    if len(message.command) < 2:
        await message.reply_text(quote_text("Usage: /add_admin [user_id]"), parse_mode=pyrogram.enums.ParseMode.HTML)
        return
    try:
        user_id = int(message.command[1])
...
async def shortlink_cmd(client, message):
    ...
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /shortlink [url] [api]"), parse_mode=pyrogram.enums.ParseMode.HTML)
        return
