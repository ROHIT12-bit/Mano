...
async def meta_cmd(client: Client, message: Message):
    if len(message.command) < 2:
        await message.reply_text(f"Usage: /meta [text]\nExample: /meta {Config.CREDITS_LINE}")
        return
    text = message.text.split(" ", 1)[1]
    await db.set_metadata(message.from_user.id, text)
    await message.reply_text(f"Metadata text set to: <code>{text}</code>\n\n{Config.CREDITS_LINE}")

@Client.on_message(filters.private & filters.command("setallmeta"))
async def setallmeta_cmd(client: Client, message: Message):
    if len(message.command) < 2:
        await message.reply_text(f"Usage: /setallmeta [text]\nExample: /setallmeta {Config.CREDITS_LINE}")
        return
    text = message.text.split(" ", 1)[1]
    await db.set_metadata(message.from_user.id, text, status=True)
    await message.reply_text(f"Metadata text set to: <code>{text}</code> and enabled.\n\n{Config.CREDITS_LINE}")
...
async def metaon_cmd(client: Client, message: Message):
    ...
    await message.reply_text(f"Metadata enabled.\n\n{Config.CREDITS_LINE}")
...
async def metaoff_cmd(client: Client, message: Message):
    ...
    await message.reply_text(f"Metadata disabled.\n\n{Config.CREDITS_LINE}")
