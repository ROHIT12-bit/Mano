...
async def premium_info(client: Client, message: Message):
    ...
    if is_premium:
        ...
        await message.reply_text(f"You are a premium user. Your plan expires on: <code>{readable_expiry}</code>\n\n{Config.CREDITS_LINE}")
    else:
        await message.reply_text(f"You are not a premium user. Use /plan to see our plans.\n\n{Config.CREDITS_LINE}")
...
async def add_credit_cmd(client, message):
    ...
    try:
        ...
        await message.reply_text(f"Added {amount} credits to {user_id}\n\n{Config.CREDITS_LINE}")
    except ValueError:
...
async def rem_credit_cmd(client, message):
    ...
    try:
        ...
        amount = int(message.command[2])
        await db.add_credits(user_id, -amount)
        await message.reply_text(f"Removed {amount} credits from {user_id}\n\n{Config.CREDITS_LINE}")
    except ValueError:
        await message.reply_text("Invalid input.")
