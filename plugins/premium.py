...
async def trial_cmd(client, message):
    ...
    if await db.is_premium(user_id):
        await message.reply_text(quote_text("You already have premium!"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    if await db.has_used_trial(user_id):
        await message.reply_text(quote_text("You have already used your trial!"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
...
async def add_credit_cmd(client, message):
    ...
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /addcredit [user_id] [amount]"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    try:
        ...
        await message.reply_text(quote_text("Invalid input."), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
...
async def rem_credit_cmd(client, message):
    ...
    if message.from_user.id not in Botskingdoms:
        return
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /remcredit [user_id] [amount]"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    try:
        user_id = int(message.command[1])
        amount = int(message.command[2])
...
async def add_premium_cmd(client, message):
    ...
    if message.from_user.id not in Botskingdoms:
        return
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /add_premium [user_id] [days]"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    try:
        user_id = int(message.command[1])
...
async def remove_premium_cmd(client, message):
    ...
    if len(message.command) < 2:
        await message.reply_text(quote_text("Usage: /remove_premium [user_id]"), parse_mode=pyrogram.enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    try:
        user_id = int(message.command[1])
