...
async def add_credit_cmd(client, message):
    ...
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /addcredit [user_id] [amount]"))
        return
    try:
        ...
        await message.reply_text(quote_text("Invalid input."))
...
async def rem_credit_cmd(client, message):
    ...
    if message.from_user.id not in Botskingdoms:
        return
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /remcredit [user_id] [amount]"))
        return
    try:
        user_id = int(message.command[1])
        amount = int(message.command[2])
...
async def add_premium_cmd(client, message):
    ...
    if message.from_user.id not in Botskingdoms:
        return
    if len(message.command) < 3:
        await message.reply_text(quote_text("Usage: /add_premium [user_id] [days]"))
        return
    try:
        user_id = int(message.command[1])
...
async def remove_premium_cmd(client, message):
    ...
    if len(message.command) < 2:
        await message.reply_text(quote_text("Usage: /remove_premium [user_id]"))
        return
    try:
        user_id = int(message.command[1])
