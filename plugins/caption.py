from pyrogram import Client, filters
from pyrogram.types import Message
from database.database import db
from config import Config

@Client.on_message(filters.private & filters.command("set_caption"))
async def set_caption_cmd(client: Client, message: Message):
    if len(message.command) < 2:
        await message.reply_text(f"Usage: /set_caption [caption text]\nExample: /set_caption <b>{{file_name}}</b>\n\n{Config.CREDITS_LINE}")
        return
    caption = message.text.split(" ", 1)[1]
    await db.set_caption(message.from_user.id, caption)
    await message.reply_text(f"Caption saved successfully!\n\n{Config.CREDITS_LINE}")

@Client.on_message(filters.private & filters.command("see_caption"))
async def see_caption_cmd(client: Client, message: Message):
    caption = await db.get_caption(message.from_user.id)
    if caption:
        await message.reply_text(f"<b>Your current caption:</b>\n\n<code>{caption}</code>\n\n{Config.CREDITS_LINE}")
    else:
        await message.reply_text(f"You don't have any custom caption set.\n\n{Config.CREDITS_LINE}")

@Client.on_message(filters.private & filters.command("del_caption"))
async def del_caption_cmd(client: Client, message: Message):
    await db.set_caption(message.from_user.id, None)
    await message.reply_text(f"Caption deleted successfully!\n\n{Config.CREDITS_LINE}")
